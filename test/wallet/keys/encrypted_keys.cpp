...
BOOST_AUTO_TEST_CASE(
    encrypted__fixture__unicode_passphrase__matches_encrypted_test_vector)
{
...
BOOST_AUTO_TEST_CASE(encrypted__create_token_lot__defaults__expected)
{
    ...
    BOOST_REQUIRE_EQUAL(
        encode_base58(out_token), "passphrasecpXbDpHuo8F7yQVcg1eQKPuX7rzGwBtEH1"
...
BOOST_AUTO_TEST_CASE(encrypted__create_token_lot__passphrase__expected)
{
    ...
    BC_REQUIRE_CREATE_TOKEN_LOT(passphrase, salt, lot, sequence);
    BOOST_REQUIRE_EQUAL(
        encode_base58(out_token), "passphrasecpXbDpHuo8F7x4pQXMhsJs2j7L8LTV8ujk"
                                  "9jGqgzUrafBeto9VrabP5SmvANvz");
}
...
BOOST_AUTO_TEST_CASE(encrypted__create_token_lot__passphrase_lot_max__expected)
{
    ...
    BC_REQUIRE_CREATE_TOKEN_LOT(passphrase, salt, lot, sequence);
    BOOST_REQUIRE_EQUAL(
        encode_base58(out_token), "passphrasecpXbDpHuo8FGWnwMTnTFiHSDnqyARArE2Y"
                                  "SFQzMHtCZvM2oWg2K3Ua2crKyc11");
}

BOOST_AUTO_TEST_CASE(
    encrypted__create_token_lot__passphrase_sequence_max__expected)
{
    const size_t lot = 0;
...
BOOST_AUTO_TEST_CASE(
    encrypted__create_token_lot__passphrase_lot_sequence__expected)
{
    const size_t lot = 42;
...
BOOST_AUTO_TEST_CASE(encrypted__create_token_entropy__defaults__expected)
{
    ...
    BOOST_REQUIRE_EQUAL(
        encode_base58(out_token), "passphraseqVHzjNrYRo5G6yLmJ7TQ49fKnQtsgjybNg"
                                  "NHAKBCQKoFZcTNjNJtg4oCUgtPt3");
}
...
BOOST_AUTO_TEST_CASE(encrypted__create_token_entropy__passphrase__expected)
{
    ...
    BC_CREATE_TOKEN_ENTROPY(passphrase, entropy);
    BOOST_REQUIRE_EQUAL(
        encode_base58(out_token), "passphraseqVHzjNrYRo5G6sfRB4YdSaQ2m8URnkBYS1"
                                  "UT6JBju5G5o45YRZKLDpK6J3PEGq");
}
...
BOOST_AUTO_TEST_CASE(encrypted__encrypt_private__vector_0__expected)
{
    ...
    const auto secret = base16_array(
        "cbf4b9f70470856bb4f40f80b87edb90865997ffee6df315ab166d713af433a5");
    BC_REQUIRE_ENCRYPT(
        secret, "TestingOneTwoThree", version, compression, expected);
}
...
BOOST_AUTO_TEST_CASE(
    encrypted__decrypt_private__vector_6_multiplied_lot__expected)
{
    const uint8_t expected_version = 0x00;
    const auto key = base58_array(
...
// github.com/bitcoin/bips/blob/master/bip-0038.mediawiki#ec-multiply-no-compression-lotsequence-numbers
BOOST_AUTO_TEST_CASE(
    encrypted__decrypt_private__vector_7_multiplied_lot__expected)
{
    const uint8_t expected_version = 0x00;
    const auto key = base58_array(
...
#define BC_REQUIRE_DECRYPT_POINT(key, passphrase, version)                     \
    ...
    bool out_is_compressed = true;                                             \
    BOOST_REQUIRE(                                                             \
        decrypt(out_point, out_version, out_is_compressed, key, passphrase));  \
    BOOST_REQUIRE_EQUAL(out_version, version);                                 \
    BOOST_REQUIRE(!out_is_compressed);                                         \
    const auto derived_address =                                               \
        payment_address(ec_public{out_point, out_is_compressed}, out_version)  \
            .encoded()
...
{
    const uint8_t version = 0x00;
    const auto key = base58_array("cfrm38V8aXBn7JWA1ESmFMUn6erxeBGZGAxJPY4e36S9"
                                  "QWkzZKtaVqLNMgnifETYw7BPwWC9aPD");
    BC_REQUIRE_DECRYPT_POINT(key, "MOLON LABE", version);
    BOOST_REQUIRE_EQUAL(
        encode_base16(out_point),
        "03e20f3b812f630d0374eefe776e983549d5cdde87780be45cb9def7ee339dfed4");
...
{
    const uint8_t version = 0x00;
    const auto key = base58_array("cfrm38V8G4qq2ywYEFfWLD5Cc6msj9UwsG2Mj4Z6QdGJ"
                                  "AFQpdatZLavkgRd1i4iBMdRngDqDs51");
    BC_REQUIRE_DECRYPT_POINT(key, "ΜΟΛΩΝ ΛΑΒΕ", version);
    BOOST_REQUIRE_EQUAL(
        encode_base16(out_point),
        "0215fb4e4e62fcec936920dbda69e83facfe2cc5e152fafcf474c8fa0dcf5023f3");
...
{
    const uint8_t version = 0x00;
    const auto key = base58_array("cfrm38V5Nm1mn7GxPBAGTXawqXRwE1EbR19GqsvJ9JmF"
                                  "5VKLqi8nETmULpELkQvExCGkTNCH2An");
    BC_REQUIRE_DECRYPT_POINT(key, "libbitcoin test", version);
    BOOST_REQUIRE_EQUAL(
        encode_base16(out_point),
        "02c13b65302bbbed4f7ad67bc68e928b58e7748d84091a2d42680dc52e7916079e");
...
{
    const uint8_t version = 0x00;
    const auto key = base58_array("cfrm38V5ec4E5RKwBu46Jf5zfaE54nuB1NWHpHSpgX4G"
                                  "Qqfzx7fvqm43mBHvr89pPgykDHts9VC");
    BC_REQUIRE_DECRYPT_POINT(key, "Libbitcoin BIP38 Test Vector", version);
    BOOST_REQUIRE_EQUAL(
        encode_base16(out_point),
        "02c3b28a224e38af4219cd782653250d2e4b67ed85ac342201f8f05ff909efdc52");
...
#define BC_REQUIRE_CREATE_KEY_PAIR(token, seed, version, compressed)           \
    ec_compressed out_point;                                                   \
    encrypted_private out_private;                                             \
    BOOST_REQUIRE(create_key_pair(                                             \
        out_private, out_point, token, seed, version, compressed))
...
BOOST_AUTO_TEST_CASE(encrypted__create_key_pair__bad_checksum__false)
{
    ...
    const uint8_t version = 0x00;
    const auto seed =
        base16_array("d36d8e703d8bd5445044178f69087657fba73d9f3ff211f7");
    const auto token = base58_array("passphraseo59BauW85etaRsKpbbTrEa5RRYw6bq5K"
                                    "9yrDf4r4N5fcirPdtDKmfJw9oYNoGN");
    ec_compressed out_point;
    encrypted_private out_private;
    BOOST_REQUIRE(!create_key_pair(
        out_private, out_point, token, seed, version, compression));
}
...
BOOST_AUTO_TEST_CASE(encrypted__create_key_pair__vector_8__expected)
{
    ...
    const auto seed =
        base16_array("d36d8e703d8bd5445044178f69087657fba73d9f3ff211f7");
    const auto token = base58_array("passphraseo59BauW85etaRsKpbbTrEa5RRYw6bq5K"
                                    "9yrDf4r4N5fcirPdtDKmfJw9oYNoGM");
    BC_REQUIRE_CREATE_KEY_PAIR(token, seed, version, compression);
    BOOST_REQUIRE_EQUAL(
        encode_base58(out_private),
        "6PfPAw5HErFdzMyBvGMwSfSWjKmzgm3jDg7RxQyVCSSBJFZLAZ6hVupmpn");

    ec_uncompressed decompressed;
    BOOST_REQUIRE(decompress(decompressed, out_point));
    BOOST_REQUIRE_EQUAL(
        encode_base16(decompressed),
        "04c13b65302bbbed4f7ad67bc68e928b58e7748d84091a2d42680dc52e7916079e103b"
        "d025079e984fb4439177224e48a2d9da5768d9b886d89d22c714169723a6");
}
...
BOOST_AUTO_TEST_CASE(encrypted__create_key_pair__vector_9__expected)
{
    ...
    const uint8_t version = 0x00;
    const auto seed =
        base16_array("bbeac8b9bb39381520b6873553544b387bcaa19112602230");
    const auto token = base58_array("passphraseouGLY8yjTZQ5Q2bTo8rtKfdbHz4tme7Q"
                                    "uPheRgES8KnT6pX5yxFauYhv3SVPDD");
    BC_REQUIRE_CREATE_KEY_PAIR(token, seed, version, compression);
    BOOST_REQUIRE_EQUAL(
        encode_base58(out_private),
        "6PfU2yS6DUHjgH8wmsJRT1rHWXRofmDV5UJ3dypocew56BDcw5TQJXFYfm");
    ...
    BOOST_REQUIRE_EQUAL(
        encode_base16(decompressed),
        "04c3b28a224e38af4219cd782653250d2e4b67ed85ac342201f8f05ff909efdc52858a"
        "f96a727252a99c54e871ff7bcf9b53cb74e4da1e15d9e83625e3c91222c0");
}
...
BOOST_AUTO_TEST_CASE(encrypted__create_key_pair__vector_9_compressed__expected)
{
    ...
    BOOST_REQUIRE_EQUAL(
        encode_base58(out_private),
        "6PnQ4ihgH1pxeUWa1SDPZ4xToaTdLtjebd8Qw6KJf8xDCW67ssaAqWuJkw");
    BOOST_REQUIRE_EQUAL(
        encode_base16(out_point),
        "02c3b28a224e38af4219cd782653250d2e4b67ed85ac342201f8f05ff909efdc52");
}
...
BOOST_AUTO_TEST_CASE(
    encrypted__create_key_pair__vector_9_compressed_testnet__expected)
{
    auto compression = true;
    ...
    BOOST_REQUIRE_EQUAL(
        encode_base58(out_private),
        "8FELCpEDogaLG3WkLhSVpKKravcNDZ7HAQ7jwHApt1Rn4BHqaLAfo9nrRD");
    BOOST_REQUIRE_EQUAL(
        encode_base16(out_point),
        "02c3b28a224e38af4219cd782653250d2e4b67ed85ac342201f8f05ff909efdc52");
}
...
#define BC_REQUIRE_CREATE_KEY_PAIR_CONFIRMATION(                               \
    token, seed, version, compressed)                                          \
    ec_compressed out_point;                                                   \
    encrypted_public out_public;                                               \
    encrypted_private out_private;                                             \
    BOOST_REQUIRE(create_key_pair(                                             \
        out_private, out_public, out_point, token, seed, version, compressed))

BOOST_AUTO_TEST_CASE(
    encrypted__create_key_pair_with_confirmation__bad_checksum__false)
{
    const auto seed =
        base16_array("d36d8e703d8bd5445044178f69087657fba73d9f3ff211f7");
    const auto token = base58_array("passphraseo59BauW85etaRsKpbbTrEa5RRYw6bq5K"
    ...
    encrypted_private out_private;
    BOOST_REQUIRE(!create_key_pair(
        out_private, out_public, out_point, token, seed, 0, false));
}

// generated and verified using bit2factor.com, no lot/sequence
BOOST_AUTO_TEST_CASE(
    encrypted__create_key_pair_with_confirmation__vector_8__expected)
{
    auto compression = false;
    const uint8_t version = 0x00;
    const auto seed =
    ...
    BOOST_REQUIRE_EQUAL(
        ...
        "6PfPAw5HErFdzMyBvGMwSfSWjKmzgm3jDg7RxQyVCSSBJFZLAZ6hVupmpn");
    BOOST_REQUIRE_EQUAL(
        encode_base58(out_public), "cfrm38V5Nm1mn7GxPBAGTXawqXRwE1EbR19GqsvJ9Jm"
                                   "F5VKLqi8nETmULpELkQvExCGkTNCH2An");
...
// generated and verified using bit2factor.com, no lot/sequence
BOOST_AUTO_TEST_CASE(
    encrypted__create_key_pair_with_confirmation__vector_9__expected)
{
    auto compression = false;
    const uint8_t version = 0x00;
    const auto seed =
    ...
    BOOST_REQUIRE_EQUAL(
        ...
        "6PfU2yS6DUHjgH8wmsJRT1rHWXRofmDV5UJ3dypocew56BDcw5TQJXFYfm");
    BOOST_REQUIRE_EQUAL(
        encode_base58(out_public), "cfrm38V5ec4E5RKwBu46Jf5zfaE54nuB1NWHpHSpgX4"
                                   "GQqfzx7fvqm43mBHvr89pPgykDHts9VC");
...
// generated and verified using bit2factor.com, no lot/sequence
BOOST_AUTO_TEST_CASE(
    encrypted__create_key_pair_with_confirmation__vector_9_compressed__expected)
{
    auto compression = true;
    const uint8_t version = 0x00;
    const auto seed =
    ...
    BOOST_REQUIRE_EQUAL(
        ...
        "6PnQ4ihgH1pxeUWa1SDPZ4xToaTdLtjebd8Qw6KJf8xDCW67ssaAqWuJkw");
    BOOST_REQUIRE_EQUAL(
        encode_base58(out_public), "cfrm38VUEdzHWKfUjdNjV22wyFNGgtRHYhXdBFT7fWw"
                                   "7cCJbCobryAYUThq4BbTPP15g4SeBsug");
    BOOST_REQUIRE_EQUAL(
        encode_base16(out_point),
        "02c3b28a224e38af4219cd782653250d2e4b67ed85ac342201f8f05ff909efdc52");
}
...
// altchain vectors are based on preliminary bidirectional mapping proposal.
BOOST_AUTO_TEST_CASE(
    encrypted__create_key_pair_with_confirmation__vector_9_compressed_testnet__expected)
{
    auto compression = true;
    const uint8_t version = 111;
    const auto seed =
    ...
    BOOST_REQUIRE_EQUAL(
        ...
        "8FELCpEDogaLG3WkLhSVpKKravcNDZ7HAQ7jwHApt1Rn4BHqaLAfo9nrRD");
    BOOST_REQUIRE_EQUAL(
        encode_base58(out_public), "cfrm2zc77zW4FRDALEVBoKmmT79Q7KshtvLZoN62JAD"
                                   "nXGPEcPosMx8sM8Ry4ieGW3FXUEoBwk2");
    BOOST_REQUIRE_EQUAL(
        encode_base16(out_point),
        "02c3b28a224e38af4219cd782653250d2e4b67ed85ac342201f8f05ff909efdc52");
}
...
BOOST_AUTO_TEST_CASE(
    encrypted__encrypt__compressed_testnet__matches_secret_version_and_compression)
{
    const auto secret = base16_array(
        "09c2686880095b1a4c249ee3ac4eea8a014f11e6f986d0b5025ac1f39afbd9ae");
    const auto passphrase = "passphrase";
    ...
    BOOST_REQUIRE(
        encrypt(out_private_key, secret, passphrase, version, is_compressed));
    ...
    BOOST_REQUIRE(decrypt(
        out_secret, out_version, out_is_compressed, private_key, passphrase));
...
BOOST_AUTO_TEST_CASE(
    encrypted__create_token_entropy__private_uncompressed_testnet__decrypts_with_matching_version_and_compression)
{
    // Create the token.
    encrypted_token out_token;
    const auto passphrase = "passphrase";
    ...
    const auto seed =
        base16_array("baadf00dbaadf00dbaadf00dbaadf00dbaadf00dbaadf00d");
    BOOST_REQUIRE(create_key_pair(
        out_private_key, out_point, token, seed, version, is_compressed));
...
BOOST_AUTO_TEST_CASE(
    encrypted__create_token_lot__private_and_public_compressed_testnet__decrypts_with_matching_version_and_compression)
{
    // Create the token.
    encrypted_token out_token;
    const auto passphrase = "passphrase";
    ...
    const auto seed =
        base16_array("baadf00dbaadf00dbaadf00dbaadf00dbaadf00dbaadf00d");
    BOOST_REQUIRE(create_key_pair(
        out_private_key, out_public_key, out_point, token, seed, version,
        is_compressed));
    ...
    ec_compressed compressed;
    BOOST_REQUIRE(decrypt(
        out_secret1, out_version1, out_is_compressed1, private_key,
        passphrase));
    ...
    BOOST_REQUIRE(decrypt(
        out_point2, out_version2, out_is_compressed2, public_key, passphrase));
