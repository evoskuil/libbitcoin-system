...
BOOST_AUTO_TEST_CASE(
    qr_code__encode__one_character_version_0_scale_1_margin_0__expected)
{
    static const data_chunk expected{
        ...
        0xfe, 0x5b, 0xf8, 0x82, 0x72, 0x08, 0xba, 0xda, 0xe8, 0xba, 0x52, 0xe8,
        0xba, 0x2a, 0xe8, 0x82, 0x0a, 0x08, 0xfe, 0xab, 0xf8, 0x00, 0xd8, 0x00,
        0xef, 0xf6, 0x20, 0x31, 0x02, 0x30, 0xbf, 0xa8, 0x88, 0x20, 0x02, 0x38,
        0x0e, 0xea, 0xa8, 0x00, 0x95, 0x50, 0xfe, 0x97, 0x70, 0x82, 0xbd, 0xd0,
        0xba, 0xb7, 0x68, 0xba, 0x02, 0x30, 0xba, 0x88, 0x88, 0x82, 0xe2, 0x38,
        0xfe, 0xca, 0xa8};
...
BOOST_AUTO_TEST_CASE(qr_code__encode__excessive_margin__false)
{
    ...
    BOOST_REQUIRE(
        !qr_code::encode(sink, "X", default_version, no_scale, bc::max_uint16));
}
...
BOOST_AUTO_TEST_CASE(qr_code__encode__excessive_scale__false)
{
    ...
    BOOST_REQUIRE(!qr_code::encode(
        sink, "X", default_version, bc::max_uint16, no_margin));
}
...
BOOST_AUTO_TEST_CASE(qr_code__encode__recovery_level_low__expected_size)
{
    ...
    stream::out::data sink(out);
    BOOST_REQUIRE(qr_code::encode(
        sink, "X", default_version, 1, no_margin,
        qr_code::recovery_level::low));
...
BOOST_AUTO_TEST_CASE(qr_code__encode__recovery_level_highest__expected_size)
{
    ...
    BOOST_REQUIRE(qr_code::encode(
        sink, "X", default_version, 1, no_margin,
        qr_code::recovery_level::highest));
    BOOST_REQUIRE_EQUAL(out.size(), 219u);
}
...
BOOST_AUTO_TEST_CASE(qr_code__encode__encode_mode_eight_bit__expected_size)
{
    ...
    stream::out::data sink(out);
    BOOST_REQUIRE(qr_code::encode(
        sink, "X", default_version, 1, no_margin, low,
        qr_code::encode_mode::eight_bit));
    BOOST_REQUIRE_EQUAL(out.size(), 219u);
}
...
BOOST_AUTO_TEST_CASE(qr_code__encode__encode_mode_kanji__expected_size)
{
    ...
    BOOST_REQUIRE(qr_code::encode(
        sink, "X", default_version, 1, no_margin, low,
        qr_code::encode_mode::kanji));
    BOOST_REQUIRE_EQUAL(out.size(), 219u);
}

// to_pixels

class qr_code_accessor : qr_code
{
public:
    static data_chunk to_pixels(
        const data_chunk& coded, uint32_t coded_width, uint16_t scale = 8,
        uint16_t margin = 2)
    {
...
BOOST_AUTO_TEST_CASE(qr_code__to_pixels__endianness_independent__expected)
{
    ...
    static const data_chunk data{0x01, 0xff, 0xfe, 0xfe};
...
BOOST_AUTO_TEST_CASE(qr_code__to_pixels__little_endian_dependent__expected)
{
    ...
    static const data_chunk data{0x01, 0xfe, 0xfe, 0xff};
