...
BOOST_AUTO_TEST_CASE(
    byte_reader__rewind__skip_to_end_rewind_to_middle__expected)
{
    const auto size = 42;
...
BOOST_AUTO_TEST_CASE(
    byte_reader__get_position__skip_to_end_set_to_middle__expected)
{
    const auto size = 42;
    const std::string value(size, 0x00);
    std::istringstream stream{value + "*" + value};
...
BOOST_AUTO_TEST_CASE(
    byte_reader__read_string_buffer__full_embedded_null__truncated_exhausted)
{
...
// The full number of bytes are read, and the string is terminated at null.
BOOST_AUTO_TEST_CASE(
    byte_reader__read_string_buffer__partial_embedded_null__truncated_not_exhausted)
{
    const std::string value{"abcdef\0hij"};
    const auto length = value.length();
    std::istringstream stream{value + "*"};
