...
checkpoint::checkpoint() noexcept : checkpoint({}, zero, false)
{
}
...
checkpoint::checkpoint(
    const hash_digest& hash, size_t height, bool valid) noexcept
    : hash_(hash), height_(height), valid_(valid)
{
}

// private
checkpoint checkpoint::from_string(
...
std::istream& operator>>(std::istream& input, checkpoint& out) noexcept
{
    ...
    if (tokens.size() == two && decode_hash(hash, tokens.front())
...
checkpoint tag_invoke(
    json::value_to_tag<checkpoint>, const json::value& value) noexcept
{
    hash_digest hash;
    if (!decode_hash(hash, value.at("hash").get_string().c_str()))
        return {};

    return {hash, value.at("height").to_number<size_t>()};
}
