...
props::props(const props& other) noexcept
    : type_(other.type_), value_(other.value_), children_(other.children_)
{
}

props::props() noexcept : props(type::null_)
{
}
...
props::props(int64_t number) noexcept : props(from_number(number))
{
}

props::props(const std::string& text) noexcept : props(type::string_, text)
{
}

// private
props::props(type type, const std::string& text) noexcept
    : type_(type), value_(text), children_()
{
}
...
std::ostream& props::write(
    std::ostream& stream, format format, bool flat) const noexcept
{
...
// private
std::ostream& props::write(
    std::ostream& stream, format format, size_t depth, bool flat) const noexcept
{
    const auto end = flat ? "" : ("\n" + std::string(depth * tab, ' '));
    const auto start = flat ? "" : (is_zero(depth) ? "" : end);
    ...
    case type::array_:
    {
        ...
        case props::format::json:
        {
            ...
            if (!children_.empty())
            {
                ...
                children_.back().second.write(
                    stream, format, add1(depth), flat);
            }
        ...
        case props::format::json:
        {
            ...
            if (!children_.empty())
            {
                for (auto child = children_.begin();
                     child != std::prev(children_.end()); ++child)
                    child->second.write(
                        stream, format, add1(depth), flat, child->first)
                        << ",";

                children_.back().second.write(
                    stream, format, add1(depth), flat, children_.back().first);
            }
...
object_props::object_props() noexcept : props(type::object_)
{
}
