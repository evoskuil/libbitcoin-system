...
constexpr size_t min_address_size =
    version_size + options_size + ec_compressed_size + number_keys_size
    + number_sigs_size + filter_length_size + checksum_size;
...
stealth_address::stealth_address() noexcept
    : valid_(false), version_(0), scan_key_(null_ec_compressed), spend_keys_(),
      signatures_(0), filter_()
{
}
...
stealth_address::stealth_address(
    ...
    : stealth_address(
        from_stealth(filter, scan_key, spend_keys, signatures, version))
{
}
    ...
    uint8_t version) noexcept
{
    ...
    const auto coerced =
        maximum ? static_cast<uint8_t>(spend_keys_size) : signatures;
