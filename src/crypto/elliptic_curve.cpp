...
bool parse(
    const secp256k1_context* context, secp256k1_pubkey& out,
...
bool parse(
    const secp256k1_context* context, std::vector<secp256k1_pubkey>& out,
    ...
    const std::vector<secp256k1_pubkey>& keys)
{
    ...
    std::transform(
        keys.begin(), keys.end(), pointers.begin(),
...
bool serialize(
    const secp256k1_context* context, data_array<Size>& out,
    const secp256k1_pubkey point) noexcept
{
    auto size = Size;
...
template <size_t Size>
bool ec_add(
    const secp256k1_context* context, data_array<Size>& in_out,
...
template <size_t Size>
bool ec_negate(
    const secp256k1_context* context, data_array<Size>& in_out) noexcept
{
    secp256k1_pubkey pubkey;
    return parse(context, pubkey, in_out)
...
bool verify_signature(
    const secp256k1_context* context, const secp256k1_pubkey& point,
    const hash_digest& hash, const ec_signature& signature) noexcept
{
    const auto parsed =
        reinterpret_cast<const secp256k1_ecdsa_signature*>(signature.data());
...
bool ec_sum(ec_compressed& out, const compressed_list& points) noexcept
{
    ...
    return secp256k1_ec_pubkey_combine(
               context, &pubkey, to_pointers(keys).data(), points.size())
               == ec_success
           && serialize(context, out, pubkey);
}
    ...
    ec_signature& out, const data_slice& der_signature, bool strict) noexcept
{
    ...
    if (strict)
        return secp256k1_ecdsa_signature_parse_der(
                   context, parsed, der_signature.data(), der_signature.size())
               == ec_success;

    return ecdsa_signature_parse_der_lax(
               context, parsed, der_signature.data(), der_signature.size())
           == ec_success;
}

bool encode_signature(
    ...
    const hash_digest& hash) noexcept
{
    ...
    const auto signature =
        reinterpret_cast<secp256k1_ecdsa_signature*>(out.data());

    return (
        secp256k1_ecdsa_sign(
            context, signature, hash.data(), secret.data(),
    ...
    const hash_digest& hash) noexcept
{
    ...
    const auto result =
        ...
        && secp256k1_ecdsa_recoverable_signature_serialize_compact(
               context, out.signature.data(), &recovery_id, &signature)
               == ec_success;
