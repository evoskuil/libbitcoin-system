...
std::string serialize(
    const data_array<Size>& value, const std::string& fallback = "") noexcept;

/// Specialize data_chunk to base16 (avoids join and fallback).
inline std::string serialize(
    const data_chunk& value, const std::string& fallback = "") noexcept;
...
template <typename Value>
std::string serialize(
    const Value& value, const std::string& fallback = "") noexcept;
