...
/// Compression removes all but a last empty token (after trimming).
BC_API string_list
split(const std::string& text, bool compress = true) noexcept;
BC_API string_list split(
    const std::string& text, const std::string& delimiter, bool trim = true,
    bool compress = true) noexcept;
BC_API string_list split(
    const std::string& text, const string_list& delimiters,
    const string_list& trim_tokens, bool compress = true) noexcept;

BC_API bool trim_left(
    std::string& text,
    const string_list& trim_tokens = ascii_whitespace) noexcept;
BC_API bool trim_right(
    std::string& text,
    const string_list& trim_tokens = ascii_whitespace) noexcept;
BC_API void trim(
    std::string& text,
...
BC_API std::string trim_left_copy(
    const std::string& text,
    const string_list& trim_tokens = ascii_whitespace) noexcept;
BC_API std::string trim_right_copy(
    const std::string& text,
    const string_list& trim_tokens = ascii_whitespace) noexcept;
BC_API std::string trim_copy(
    const std::string& text,
    const string_list& trim_tokens = ascii_whitespace) noexcept;

BC_API void reduce(
    string_list& tokens, const string_list& trim_tokens = ascii_whitespace,
    bool compress = true) noexcept;
BC_API string_list reduce_copy(
    const string_list& tokens,
    const string_list& trim_tokens = ascii_whitespace,
    bool compress = true) noexcept;

BC_API size_t replace(
    std::string& text, const std::string& from, const std::string& to) noexcept;
BC_API std::string replace_copy(
    const std::string& text, const std::string& from,
    const std::string& to) noexcept;

BC_API bool ends_with(
    const std::string& text, const std::string& suffix) noexcept;
BC_API bool starts_with(
